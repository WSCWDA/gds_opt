...
NaiveGDSRunner::~NaiveGDSRunner() { cleanup(); }

bool NaiveGDSRunner::init(const std::string& file_path, std::size_t io_size, bool is_read) {
  cleanup();
...
bool NaiveGDSRunner::run(std::size_t iterations) {
  if (!initialized_) {
  ...
  for (std::size_t i = 0; i < iterations; ++i) {
    CUfileError_t cufile_status = cuFileBufRegister(device_buffer_, io_size_, 0);
    if (cufile_status.err != CU_FILE_SUCCESS) {
      set_error(cufile_error_to_string("run failed: cuFileBufRegister", cufile_status));
      return false;
    }
    ...
    if (is_read_) {
      ...
      if (io_ret < 0) {
        (void)cuFileBufDeregister(device_buffer_);
    ...
    cufile_status = cuFileBufDeregister(device_buffer_);
    if (cufile_status.err != CU_FILE_SUCCESS) {
      set_error(cufile_error_to_string("run failed: cuFileBufDeregister", cufile_status));
      return false;
    }
    metrics_.deregister_count += 1;
  }
...
void NaiveGDSRunner::set_error(const std::string& message) { last_error_ = message; }
